...
import { ArrowUpRight, TrendingUp, FolderGit2, Sparkles, Filter } from "lucide-react";
  ...
  return (
    <section className="py-16 sm:py-24 px-4 sm:px-8 lg:px-12 xl:px-14 max-w-[1550px] mx-auto space-y-8 sm:space-y-12 scroll-mt-20 w-full max-w-full overflow-hidden" id="works">
      ...
      <motion.div 
        ...
        className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 sm:gap-6 pb-6 border-b border-neutral-200 w-full max-w-full min-w-0"
      >
      ...
      <motion.div 
        ...
        className="w-full max-w-full overflow-x-auto pb-2 scrollbar-none flex items-center gap-2 touch-pan-x min-w-0"
      >
        ...
        {CATEGORIES.map((cat) => {
          ...
          return (
            <motion.button
              ...
              className={`px-3.5 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs font-mono tracking-wide transition-all whitespace-nowrap flex items-center gap-1.5 sm:gap-2 shrink-0 cursor-pointer ${
                isActive
      ...
      {/* Case Studies Grid with AnimatePresence & Motion */}
      <motion.div 
        layout
        className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 w-full max-w-full min-w-0"
      >
        <AnimatePresence mode="popLayout">
          {filteredCaseStudies.map((study, idx) => (
            <motion.div
              ...
              className="group cursor-pointer rounded-3xl bg-white border border-neutral-200 hover:border-black transition-colors duration-300 overflow-hidden flex flex-col justify-between shadow-sm hover:shadow-2xl w-full min-w-0"
            >
